-- ============================================
-- Migration 123 : Admin peut créer des invitations pour tous les rôles
-- - Extend type_role pour inclure 'pasteur'
-- - Admin/super_admin peut inviter Disciple, Mentor, Superviseur, Pasteur
-- - Famille obligatoire pour Superviseur (côté RPC on garde flexibilité, le front impose)
-- ============================================

-- Autoriser type_role = 'pasteur' dans invitations_famille
ALTER TABLE invitations_famille DROP CONSTRAINT IF EXISTS invitations_famille_type_role_check;
ALTER TABLE invitations_famille ADD CONSTRAINT invitations_famille_type_role_check
  CHECK (type_role IN ('disciple', 'mentor', 'superviseur', 'pasteur'));

-- ============================================
-- RPC creer_invitation_famille : ajouter support admin + pasteur
-- ============================================
...
BEGIN
  ...
  -- Admin/Super_admin : peut créer une invitation pour n'importe quel rôle
  IF v_is_admin THEN
...
COMMENT ON FUNCTION creer_invitation_famille(TEXT, UUID, TEXT) IS 'Crée une invitation. Admin/super_admin peut inviter tout rôle. Sinon: mentor/pilier/superviseur pour disciple, superviseur pour mentor, pasteur pour superviseur. SECURITY DEFINER.';
