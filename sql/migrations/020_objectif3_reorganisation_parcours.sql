-- ============================================
-- OBJECTIF 3: Réorganisation des parcours selon la liste fournie
-- Supprime les anciens parcours et crée les nouveaux
-- ============================================

-- Vérifier et corriger la structure de la table si nécessaire
DO $$
BEGIN
  -- Si titre existe mais pas nom, renommer
  IF EXISTS (
  ...
  -- Si les deux existent, supprimer titre
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'parcours_transformation' AND column_name = 'titre'
  ) AND EXISTS (
    ...
    UPDATE parcours_transformation SET nom = COALESCE(nom, titre) WHERE nom IS NULL OR nom = '';
    ALTER TABLE parcours_transformation DROP COLUMN titre;
  END IF;

  -- S'assurer que modules_parcours a bien la colonne contenu (selon l'état réel de la base)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
  ...
  -- S'assurer que modules_parcours a bien les colonnes utilisées par les INSERT de cette migration
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- Supprimer les anciens parcours et leurs modules
-- D'abord supprimer les modules pour éviter les erreurs de clé étrangère
DELETE FROM modules_parcours;
DELETE FROM parcours_transformation;

-- S'assurer qu'il n'y a pas de contrainte UNIQUE qui pourrait bloquer
DO $$
BEGIN
  -- Supprimer la contrainte UNIQUE si elle existe (on la recréera après)
  IF EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'parcours_transformation_nom_key'
  ) THEN
    ALTER TABLE parcours_transformation DROP CONSTRAINT parcours_transformation_nom_key;
  END IF;
END $$;

-- 1. PARCOURS: Jésus guérit les cœurs brisés
...
VALUES (
  ...
  '[
    ...
    {"objectif": "Expérimenter la guérison divine", "points": 2}
  ]'::jsonb,
  'actif',
  1,
  'restauration_ame'
...
-- Module pour "Jésus guérit les cœurs brisés"
...
-- 2. PARCOURS: Comment guérir un cœur brisé ?
INSERT INTO parcours_transformation (nom, description, thematique, duree_jours, niveau, objectifs, statut, ordre_affichage, categorie)
VALUES (
  ...
  'debutant',
  '[
    {"objectif": "Identifier les blessures", "points": 1},
    {"objectif": "Appliquer les principes de guérison", "points": 2},
    {"objectif": "Marcher dans la guérison", "points": 3}
  ]'::jsonb,
  'actif',
  2,
  'restauration_ame'
...
-- Module pour "Comment guérir un cœur brisé ?"
INSERT INTO modules_parcours (parcours_id, titre, description, contenu, type_contenu, duree_estimee, ordre, statut)
SELECT 
  p.id,
  'Comment guérir un cœur brisé ?',
...
5. **Marcher dans la guérison**
  ...
  'actif'
FROM parcours_transformation p
WHERE p.thematique = 'Comment guérir un cœur brisé ?'
ON CONFLICT DO NOTHING;

-- 3. PARCOURS: Jésus restaure les Finances
INSERT INTO parcours_transformation (nom, description, thematique, duree_jours, niveau, objectifs, statut, ordre_affichage, categorie)
VALUES (
  ...
  'intermediaire',
  '[
    {"objectif": "Comprendre la restauration financière", "points": 1},
...
-- Module pour "Jésus restaure les Finances"
INSERT INTO modules_parcours (parcours_id, titre, description, contenu, type_contenu, duree_estimee, ordre, statut)
SELECT 
  p.id,
  'Jésus restaure les Finances',
...
-- 4. PARCOURS: Pourquoi Dieu veut que tu prospères dans tes finances ?
INSERT INTO parcours_transformation (nom, description, thematique, duree_jours, niveau, objectifs, statut, ordre_affichage, categorie)
VALUES (
  ...
  'intermediaire',
  '[
    {"objectif": "Comprendre le cœur de Dieu pour la prospérité", "points": 1},
    {"objectif": "Découvrir les raisons bibliques", "points": 2}
  ]'::jsonb,
  'actif',
  4,
  'finances'
...
-- Module pour "Pourquoi Dieu veut que tu prospères dans tes finances ?"
INSERT INTO modules_parcours (parcours_id, titre, description, contenu, type_contenu, duree_estimee, ordre, statut)
SELECT 
  p.id,
  'Pourquoi Dieu veut que tu prospères dans tes finances ?',
...
4. **Pour être libre**
  ...
  'actif'
FROM parcours_transformation p
WHERE p.thematique = 'Pourquoi Dieu veut que tu prospères dans tes finances ?'
ON CONFLICT DO NOTHING;

-- 5. PARCOURS: Comment prospérer dans mes finances ?
INSERT INTO parcours_transformation (nom, description, thematique, duree_jours, niveau, objectifs, statut, ordre_affichage, categorie)
VALUES (
  ...
  'intermediaire',
  '[
    {"objectif": "Appliquer les principes de prospérité", "points": 1},
    {"objectif": "Gérer sagement ses finances", "points": 2},
    {"objectif": "Établir un plan de prospérité", "points": 3}
  ]'::jsonb,
  'actif',
  5,
  'finances'
...
-- Module pour "Comment prospérer dans mes finances ?"
INSERT INTO modules_parcours (parcours_id, titre, description, contenu, type_contenu, duree_estimee, ordre, statut)
SELECT 
  p.id,
  'Comment prospérer dans mes finances ?',
...
5. **Croire et déclarer**
  ...
  'actif'
FROM parcours_transformation p
WHERE p.thematique = 'Comment prospérer dans mes finances ?'
ON CONFLICT DO NOTHING;

-- 6. PARCOURS: Comment sortir des forteresses ?
INSERT INTO parcours_transformation (nom, description, thematique, duree_jours, niveau, objectifs, statut, ordre_affichage, categorie)
VALUES (
  ...
  'intermediaire',
  '[
    {"objectif": "Identifier les forteresses", "points": 1},
    {"objectif": "Utiliser les armes spirituelles", "points": 2},
    {"objectif": "Marcher dans la liberté", "points": 3}
  ]'::jsonb,
  'actif',
  6,
  'marcher_esprit'
...
-- Module pour "Comment sortir des forteresses ?"
INSERT INTO modules_parcours (parcours_id, titre, description, contenu, type_contenu, duree_estimee, ordre, statut)
SELECT 
  p.id,
  'Comment sortir des forteresses ?',
...
-- 7. PARCOURS: Comment crucifier la chair ?
INSERT INTO parcours_transformation (nom, description, thematique, duree_jours, niveau, objectifs, statut, ordre_affichage, categorie)
VALUES (
  ...
  'intermediaire',
  '[
    {"objectif": "Comprendre ce qu''est la chair", "points": 1},
    {"objectif": "Marcher selon l''Esprit", "points": 2},
    {"objectif": "Pratiquer la discipline", "points": 3}
  ]'::jsonb,
  'actif',
  7,
  'marcher_esprit'
...
-- Module pour "Comment crucifier la chair ?"
INSERT INTO modules_parcours (parcours_id, titre, description, contenu, type_contenu, duree_estimee, ordre, statut)
SELECT 
  p.id,
  'Comment crucifier la chair ?',
...
4. **Renouveler votre esprit**
  ...
  'actif'
FROM parcours_transformation p
WHERE p.thematique = 'Comment crucifier la chair ?'
ON CONFLICT DO NOTHING;

-- 8. PARCOURS: Pourquoi et Comment suivre Jésus ?
INSERT INTO parcours_transformation (nom, description, thematique, duree_jours, niveau, objectifs, statut, ordre_affichage, categorie)
VALUES (
...
-- Module pour "Pourquoi et Comment suivre Jésus ?"
INSERT INTO modules_parcours (parcours_id, titre, description, contenu, type_contenu, duree_estimee, ordre, statut)
SELECT 
  p.id,
  'Pourquoi et Comment suivre Jésus ?',
...
VALUES (
  ...
  'debutant',
  '[
    {"objectif": "Comprendre le plus grand commandement", "points": 1},
    {"objectif": "Apprendre à aimer Dieu", "points": 2}
  ]'::jsonb,
  'actif',
  9,
  'discipolat'
...
-- Module pour "Comment aimer Dieu ?"
INSERT INTO modules_parcours (parcours_id, titre, description, contenu, type_contenu, duree_estimee, ordre, statut)
SELECT 
  p.id,
  'Comment aimer Dieu ?',
...
- L''amour pour les autres',
  ...
  'actif'
FROM parcours_transformation p
WHERE p.thematique = 'Comment aimer Dieu ?'
ON CONFLICT DO NOTHING;

-- 10. PARCOURS: C'est quoi le Royaume de Dieu ?
INSERT INTO parcours_transformation (nom, description, thematique, duree_jours, niveau, objectifs, statut, ordre_affichage, categorie)
VALUES (
  ...
  'debutant',
  '[
    {"objectif": "Comprendre le Royaume de Dieu", "points": 1},
    {"objectif": "Apprendre à y entrer", "points": 2}
  ]'::jsonb,
  'actif',
  10,
  'fondements_royaume'
...
-- Module pour "C'est quoi le Royaume de Dieu ?"
INSERT INTO modules_parcours (parcours_id, titre, description, contenu, type_contenu, duree_estimee, ordre, statut)
SELECT 
  p.id,
  'C''est quoi le Royaume de Dieu ?',
...
3. **Vivre comme citoyen du Royaume**
  ...
  'actif'
FROM parcours_transformation p
WHERE p.thematique = 'C''est quoi le Royaume de Dieu ?'
ON CONFLICT DO NOTHING;

-- 11. PARCOURS: Comment faire des disciples de Christ ?
INSERT INTO parcours_transformation (nom, description, thematique, duree_jours, niveau, objectifs, statut, ordre_affichage, categorie)
VALUES (
  ...
  'intermediaire',
  '[
    {"objectif": "Comprendre le mandat de Jésus", "points": 1},
    {"objectif": "Apprendre les étapes du discipulat", "points": 2},
    {"objectif": "Pratiquer le discipulat", "points": 3}
  ]'::jsonb,
  'actif',
  11,
  'discipolat'
...
-- Module pour "Comment faire des disciples de Christ ?"
INSERT INTO modules_parcours (parcours_id, titre, description, contenu, type_contenu, duree_estimee, ordre, statut)
SELECT 
  p.id,
  'Comment faire des disciples de Christ ?',
...
- Multiplication',
  ...
  'actif'
FROM parcours_transformation p
WHERE p.thematique = 'Comment faire des disciples de Christ ?'
ON CONFLICT DO NOTHING;

-- 12. PARCOURS: Comment avoir le cœur de Dieu ?
INSERT INTO parcours_transformation (nom, description, thematique, duree_jours, niveau, objectifs, statut, ordre_affichage, categorie)
VALUES (
  ...
  'intermediaire',
  '[
    {"objectif": "Comprendre le cœur de Dieu", "points": 1},
    {"objectif": "Développer le cœur de Dieu", "points": 2},
    {"objectif": "Pratiquer l''amour", "points": 3}
  ]'::jsonb,
  'actif',
  12,
  'discipolat'
...
-- Module pour "Comment avoir le cœur de Dieu ?"
INSERT INTO modules_parcours (parcours_id, titre, description, contenu, type_contenu, duree_estimee, ordre, statut)
SELECT 
  p.id,
  'Comment avoir le cœur de Dieu ?',
...
4. **Voir comme Dieu voit**
  ...
  'actif'
FROM parcours_transformation p
WHERE p.thematique = 'Comment avoir le cœur de Dieu ?'
ON CONFLICT DO NOTHING;

-- 13. PARCOURS: Qu'est-ce qu'un disciple de Christ ?
INSERT INTO parcours_transformation (nom, description, thematique, duree_jours, niveau, objectifs, statut, ordre_affichage, categorie)
VALUES (
  ...
  'debutant',
  '[
    {"objectif": "Comprendre ce qu''est un disciple", "points": 1},
    {"objectif": "Découvrir les caractéristiques", "points": 2}
  ]'::jsonb,
  'actif',
  13,
  'discipolat'
...
-- Module pour "Qu'est-ce qu'un disciple de Christ ?"
INSERT INTO modules_parcours (parcours_id, titre, description, contenu, type_contenu, duree_estimee, ordre, statut)
SELECT 
  p.id,
  'Qu''est-ce qu''un disciple de Christ ?',
...
- Servir et aimer les autres',
  ...
  'actif'
FROM parcours_transformation p
WHERE p.thematique = 'Qu''est-ce qu''un disciple de Christ ?'
ON CONFLICT DO NOTHING;

-- Ajouter une contrainte UNIQUE sur nom pour éviter les doublons futurs
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
...
BEGIN
  ...
  SELECT COUNT(DISTINCT nom) INTO unique_count FROM parcours_transformation WHERE statut = 'actif';
  
  RAISE NOTICE '========================================';
  RAISE NOTICE '✅ PARCOURS RÉORGANISÉS';
  RAISE NOTICE '========================================';
  RAISE NOTICE 'Parcours actifs: %', parcours_count;
  RAISE NOTICE 'Parcours uniques: %', unique_count;
  RAISE NOTICE 'Modules actifs: %', modules_count;
  IF parcours_count != unique_count THEN
    RAISE WARNING 'ATTENTION: Il y a des doublons !';
  END IF;
  RAISE NOTICE '========================================';
END $$;
