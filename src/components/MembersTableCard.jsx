...
/**
 * Tableau de membres réutilisable : en-tête (titre, description), Recherche et Filtres,
 * tableau (Photo, Nom, Email, Statut, Nombre de Disciples, Progression, Suivi par, Date),
 * pagination, exports Excel/PDF, sélection.
 * Utilisable sur tous les dashboards (Superviseur, Pasteur, Mentor, Disciple).
 */
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <CardTitle className="text-lg font-semibold text-gray-900 flex items-center gap-2">
              ...
              {title} ({filteredMembres?.length ?? 0})
            </CardTitle>
            <CardDescription className="mt-1">{description}</CardDescription>
          </div>
          <div className="flex gap-2">
            {showSelection && selectedMembres?.length > 0 && (
        ...
        {paginatedMembres?.length > 0 ? (
              ...
              <Table className="min-w-[900px]">
                <TableHeader>
                  <TableRow className="bg-purple-200 hover:bg-purple-300 text-gray-900">
                    ...
                    {showProgression && <TableHead>Progression</TableHead>}
                    {showSuiviPar && <TableHead>Suivi par</TableHead>}
                    ...
                    <TableRow key={membre.id} className={`hover:bg-gray-50 hover:text-black ${selectedMembres?.includes(membre.id) ? 'bg-blue-50' : ''}`}>
                      ...
                      {showNombreDisciples && (
                        <TableCell className="text-center hover:text-black">
                          {showFetchDisciples && onFetchDisciples ? (
                            <button
                              type="button"
                            ...
                            <span className="inline-flex items-center justify-center rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-sm font-semibold text-blue-700">
                              {membre.nombreDisciples ?? 0}
                            </span>
                          )}
                        </TableCell>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">
              {searchTerm || statusFilter !== 'tous' || dateFilter || (showProgression && progressionFilter !== 'tous')
